    ...
    public class AccountController : Controller
    {
        public void LoadSession()
        {
            ...
            try
            {
                if (Session["isAdmin"].ToString() == "")
                {
                    ...
                    string strQuery = @"SELECT  isAdmin
                                FROM
	                                tblUserInfo
                                WHERE
	                                UserName = '" + User.Identity.Name + "' and isActive='True'";
                    DataTable dt = dbAccess.SQLViewerData(strQuery);

                    if (dt.Rows.Count > 0)
                    {
                        Session["isAdmin"] = dt.Rows[0][0].ToString();
                    }
                    flag = 1;
                }
        ...
        public ActionResult ExternalLoginConfirmation(RegisterExternalLoginModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                using (SQLDataViewerAuthenticationEntities db = new SQLDataViewerAuthenticationEntities())
                {
